...
def build_input_multi_batchsize(dataset, data_path, batch_size, mode):
  """Build CIFAR image and labels.
  Args:
    dataset: Either 'cifar10' or 'cifar100'.
    data_path: Filename for data.
  ...
  if mode == 'train':
    ...
    example_queue = tf.RandomShuffleQueue(
        ...
        min_after_dequeue=2 * max_batch_size,
        dtypes=[tf.float32, tf.int32],
        shapes=[[image_size, image_size, depth], [1]])
    num_threads = 4
  else:
  ...
  for i in range(1, 2048):
    ...
    assert len(images.get_shape()) == 4
    assert images.get_shape()[0] == i
    assert images.get_shape()[-1] == 3
    assert len(labels.get_shape()) == 2
    assert labels.get_shape()[0] == i
    assert labels.get_shape()[1] == num_classes

    input_variable_batchsize.append([images, labels])

  return input_variable_batchsize
